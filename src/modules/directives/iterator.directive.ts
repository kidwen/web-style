...
export class IteratorDirective<T, U extends NgIterable<T> = NgIterable<T>>
    implements DoCheck {
    @Input("styleForOf")
    ...
    public ngDoCheck(): void {
        if (this.differ) {
            const change: IterableChanges<T> | null = this.differ.diff(
                this.dataSource,
            );
            if (change) {
            ...
            (
                ...
                if (item.previousIndex === null) {
                    this.container.createEmbeddedView(
                        this.template,
                        new IteratorContext<T, U>(null!, 0),
                        currentIndex === null ? undefined : currentIndex,
                    );
                    const tuple: IteratorContext<T, U> = new IteratorContext<
                        T,
                        U
                    >(item, 0);
                    ...
                    const view: ViewRef = this.container.get(
                        adjustedPreviousIndex,
                    )!;
                    this.container.move(view, currentIndex);
                    const tuple: IteratorContext<
                        unknown,
                        NgIterable<unknown>
                    > = new IteratorContext(item, adjustedPreviousIndex);
                    insertTuples.push(tuple);
                }
            },
        );
